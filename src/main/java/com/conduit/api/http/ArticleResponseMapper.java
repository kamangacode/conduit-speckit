...
public final class ArticleResponseMapper {
    ...
    public static ArticleListResponse toListResponse(List<Article> articles, long total, UserRepository users) {
        return new ArticleListResponse(articles.stream().map(article -> toPayload(article, users, false)).toList(), total);
    }
    ...
    private static Payload toPayload(Article article, UserRepository users, boolean includeBody) {
        ...
        return new Payload(article.slug(), article.title(), article.description(),
                includeBody ? article.body() : null, article.tagValues(), article.createdAt(), article.updatedAt(),
                new Author(author.username(), author.bio(), author.image(), false), false, 0);
    }

    public record ArticleResponse(Payload article) { }
    public record ArticleListResponse(List<Payload> articles, long articlesCount) { }
    public record Payload(String slug, String title, String description,
                          @JsonInclude(JsonInclude.Include.NON_NULL) String body, List<String> tagList,
                          Instant createdAt, Instant updatedAt, Author author, boolean favorited, int favoritesCount) { }
