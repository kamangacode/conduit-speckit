...
public class UserController {
  ...
  public UserController(
      ...
      GetCurrentUserUseCase current,
      UpdateCurrentUserUseCase update,
      TokenService tokens) {
  ...
  public ResponseEntity<UserResponseMapper.UserResponse> register(@RequestBody JsonNode body) {
    ...
    var created =
        register.execute(text(user, "email"), text(user, "username"), text(user, "password"));
    return ResponseEntity.status(HttpStatus.CREATED)
        .body(UserResponseMapper.toResponse(created, tokens));
  }
  ...
  public UserResponseMapper.UserResponse login(@RequestBody JsonNode body) {
    ...
    return UserResponseMapper.toResponse(
        login.execute(text(user, "email"), text(user, "password")), tokens);
  }
  ...
  @PutMapping("/user")
  public UserResponseMapper.UserResponse update(
      @RequestBody JsonNode body, HttpServletRequest request) {
    JsonNode user = requiredUser(body);
    var command =
        new UpdateCurrentUserUseCase.UpdateCommand(
  ...
  private static UUID userId(HttpServletRequest request) {
    ...
    throw new com.conduit.application.user.UserException(
        "token", "is missing", com.conduit.application.user.UserException.ErrorKind.UNAUTHORIZED);
  }

  private static JsonNode requiredUser(JsonNode body) {
    if (body == null || !body.has("user") || !body.get("user").isObject()) {
      throw new com.conduit.application.user.UserException(
          "user", "is required", com.conduit.application.user.UserException.ErrorKind.VALIDATION);
    }
