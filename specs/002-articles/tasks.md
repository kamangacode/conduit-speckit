...
- [ ] T059 [HIGH] [FR-013] Add `.gitleaks.toml` and persistent OWASP Dependency-Check configuration in the deferred `security` Maven profile, then execute the NVD scan later in an environment providing `NVD_API_KEY` without committing secrets (deferred: Gitleaks passed)
...
- [ ] T062 [MEDIUM] [SC-001--SC-006] Execute the reproducible `quickstart.md` sequence in order and record the final validation result; retain the NVD scan as an explicitly deferred security gate and isolate iteration-3 endpoints from the article contract run (partial)