...
- [ ] T059 [HIGH] [FR-013] Add `.gitleaks.toml` and persistent OWASP Dependency-Check configuration in the `security` Maven profile, then execute both scans in an environment providing the required tools and NVD credentials without committing secrets (partial: Gitleaks passed; NVD scan remains blocked)
